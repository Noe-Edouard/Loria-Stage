...
def frangi_filter(image, scales=range(1, 10, 2), alpha=0.5, beta=0.5, gamma=None, mode='reflect', cval=0):
    ...
    for scale in scales:
        hess = hessian_matrix(image, scale, mode=mode, cval=cval, use_gaussian_derivatives=True) # list of tensors [Hxx, Hxy, Hxz, ...] with Hxx of shape (X, Y, Z)
        hess = [h.astype(np.float32) for h in hess]
        
        eigvals = hessian_matrix_eigvals(hess) 
        eigvals = np.sort(np.abs(eigvals), axis=0) ### a changer
