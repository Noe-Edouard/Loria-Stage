...
from core.config import HessianConfig, EnhancementConfig, ProcessingConfig
...
class Enhancer:
    ...
    def enhance_data(
        self, 
        data: np.ndarray, 
        method: Literal['frangi', 'default'],
        processing_params: ProcessingConfig,
        enhancement_params: EnhancementConfig, 
        hessian_params: HessianConfig,
    ) -> np.ndarray:
        # Prepare params
    ...
    def _prepare_enhancement(
        ...
        enhancement_function = self.selector[method]
    ...
    def _process_data(
        ...
        if parallelize and data.ndim == 3:
            ...
            self.logger.info("Parallel processing in progress...")
            with ProgressBar():
                result = processed_chunks.compute()
        
        # Sequential processing (or 2D)
        else:
            self.logger.info("Sequential processing in progress...")
