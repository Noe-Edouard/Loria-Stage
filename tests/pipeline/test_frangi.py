...
def test_frangi():
    ...
    viewer = Viewer()
    saver = Saver(experiment_name="test_frangi", output_dir='test', logger=logger)
    ...
    figure_2d = viewer.display_images([image_2d, result_2d_own, result_2d_skimage], ['Original', 'Enhanced (frangi own)', 'Enhanced (frangi skimage)'])
    ...
    saver.save_plot(figure_2d, 'test_frangi_2d_comparison')
    saver.save_plot(histogram_2d, 'test_frangi_2d_histogram')
    ...
    figure_3d = viewer.display_slices([image_3d, result_3d_own, result_3d_skimage], ['Original', 'Enhanced (own)', 'Enhanced (skimage)'])
    ...
    saver.save_animation(figure_3d, 'test_frangi_3d_comparison')
    saver.save_plot(histogram_3d, 'test_frangi_3d_histogram')
    
    assert np.max(np.abs(result_3d_own - result_3d_skimage)) < 1e-2
